...
(L<Perlmonks|http://www.perlmonks.org/>) is probably one of the oldest Perl related site. This community site provides a way to the Perl zealots (that humorously call themselves 'monks') to exchange on all aspects of the language. More than the rating system (users can 'vote' on other users's posts) which is only a rough indication of the user involvement on the site, you might want to consider the powerful search engine to get a good overview of one user's interests/proficiency.
...
=head2 Perl proficiency

Determining Proficiency is hard, and determining Perl proficiency might be even harder:
There's no bullet proof solution to assert proficiency during an interview, but the community has come up with some questions to help L<identify good perl programmers|http://modernperlbooks.com/mt/2011/01/how-to-identify-a-good-perl-programmer.html>
